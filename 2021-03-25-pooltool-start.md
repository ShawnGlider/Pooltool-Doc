...
In the [first](2020-04-24-pooltool-theory.html) and [second](2020-12-20-pooltool-alg.html) posts of this series, I discussed _ad nauseam_ the physics and algorithmic theory behind pool simulation. With this all now behind me, it's time to **take this theory to the streets**.

![before_after](assets/simulator/before_after.gif){: style="width:70%; display: block; margin: 1em auto; border-radius:8px;"}
...
Of these attributes, the most important is `rvw`, which stores the [ball state](2020-12-20-pooltool-alg.html#what-is-the-system-state) as a $3 \times 3$ `numpy` array. `rvw` is named after the 3 state vectors $\vec{r}(t)$, $\vec{v}(t)$, and $\vec{\omega}(t)$.
...
This method ultimately calls upon [`physics.cue_strike`](https://github.com/ekiefl/pooltool/blob/51552ff7704376682359059b5dbd8a093f4ded17/psim/physics.py#L102), which implements the cue-ball interaction physics described [here](2020-04-24-pooltool-theory.html#section-vi-ball-cue-interactions).
</div>
...
Rather than implementing the event-based shot evolution algorithm I [wouldn't shut up about](2020-12-20-pooltool-alg.html#continuous-event-based-evolution), I implemented some dinky discrete time evolution algorithm just to get the _ball rolling_. It's a for loop that increments by $50 \text{ms}$.
...
![ball_traj_0](assets/simulator/ball_traj_0.png){: style="width:70%; display: block; margin: 1em auto; border-radius:8px;"}

Immediately after being hit, the ball is [sliding](2020-04-24-pooltool-theory.html#case-4-sliding). Yet after a short amount of time, the relative velocity converges to $\vec{0}$, which defines the transition from sliding to [rolling](2020-04-24-pooltool-theory.html#case-3-rolling). Once rolling, the ball continues to roll until it reaches the [stationary](2020-04-24-pooltool-theory.html#case-1-stationary) state.
...
![ball_traj_0](assets/simulator/ball_traj_0.gif){: style="width:70%; display: block; margin: 1em auto; border-radius:8px;"}
...
![ball_traj_2](assets/simulator/ball_traj_2.gif){: style="width:70%; display: block; margin: 1em auto; border-radius:8px;"}
...
<video controls 
    src="YouTubeEmbedded/11.Super Trick Shots with Mezz Masse Cue by Florian Venom Kohler HD.mp4" 
    style="width:800px; display:block; margin:0 auto;">
</video>
<p style="text-align:center; color:#666;">Super Trick Shots with Mezz Masse Cue by Florian Venom Kohler
  <a href="https://www.youtube.com/watch?v=t_ms6KjSoS8"; style="text-decoration: underline">Youtube Video</a>
</p>
...
![comparison](assets/simulator/comparison.gif){: style="width:70%; display: block; margin: 1em auto; border-radius:8px;"}
...
$4400$ RPM. That's... that's too much, right? Well, the same guy put out [this](YouTubeEmbedded/12.HOW%20FAST%20is%20the%20Cue%20Ball%20spinning%20(in%20RPMs).mp4) video where he purports his RPM for a different shot to be $3180$. So I'm certainly in the ball park. Maybe he can get up to $4400$ RPM, or maybe my simulated cloth had a higher coefficient of sliding friction, requiring higher RPM.
...
So far I've been evolving the simulation by incrementing time in small discrete steps (_aka_ a [discrete time evolution algorithm](2020-12-20-pooltool-alg.html#discrete-time-evolution). Yet moving forward, I've opted to use the event-based evolution algorithm for its superior accuracy and computational efficiency.

The premise of the algorithm is this:

- We have beautiful equations of motion for each ball that collectively describe the evolution of the [system state](2020-12-20-pooltool-alg.html#what-is-the-system-state). Great.
...
- So the algorithm works by evolving the system state directly up until the next event, at which time the event must be resolved (_e.g._ a [ball-ball collision event](2020-12-20-pooltool-alg.html#ball-ball-collision) is resolved by applying the [ball-ball interaction equations](2020-04-24-pooltool-theory.html#section-ii-ball-ball-interactions)), and then the process repeats itself: the next event is found and the system state is evolved up until the next event.
...
All events are either **transitions**, or they are **collisions** (details [here](2020-12-20-pooltool-alg.html#2-what-are-events)). Since there are no collisions yet, I decided to implement the algorithm using just transition events to start. Transition events mark the transitioning of a ball from one motion state to another (_e.g._ from [rolling](2020-04-24-pooltool-theory.html#case-3-rolling) to [stationary](2020-04-24-pooltool-theory.html#case-1-stationary)).
...
![straight_shot_discrete](assets/simulator/straight_shot_discrete.gif){: style="width:70%; display: block; margin: 1em auto; border-radius:8px;"}
...
![cts_1](assets/simulator/cts_1.gif){: style="width:70%; display: block; margin: 1em auto; border-radius:8px;"}
...
![cts_2](assets/simulator/cts_2.gif){: style="width:70%; display: block; margin: 1em auto; border-radius:8px;"}
...
![cts_3](assets/simulator/cts_3.gif){: style="width:70%; display: block; margin: 1em auto; border-radius:8px;"}
...
![snapshot_1_2](assets/simulator/snapshot_1_2.jpg){: style="width:70%; display: block; margin: 1em auto; border-radius:8px;"}
...
![snapshot_2](assets/simulator/snapshot_2.png){: style="width:70%; display: block; margin: 1em auto; border-radius:8px;"}


### Comparing the algorithms

How do these results compare to the discrete time algorithm? With discrete time, collisions are [detected retrospectively](2020-12-20-pooltool-alg.html#discrete-time-evolution) by seeing if there is any overlapping geometry. This leads to an inherent inaccuracy. Sure, it can be reduced by **decreasing the time step**. But how small does the timestep have to be and **what effect does this have on performance**?
...
![slight_graze](assets/simulator/slight_graze.gif){: style="width:70%; display: block; margin: 1em auto; border-radius:8px;"}
...
![discrete_bad](assets/simulator/discrete_bad.gif){: style="width:70%; display: block; margin: 1em auto; border-radius:8px;"}
...
![discrete_comp](assets/simulator/discrete_comp.png){: style="width:70%; display: block; margin: 1em auto; border-radius:8px;"}
...
![event_heatmap](assets/simulator/event_heatmap.png){: style="width:70%; display: block; margin: 1em auto; border-radius:8px;"}
