...
In the [first](2020-04-24-pooltool-theory.md) and [second](2020-12-20-pooltool-alg.md) posts of this series, I discussed _ad nauseam_ the physics and algorithmic theory behind pool simulation. With this all now behind me, it's time to **take this theory to the streets**.
...
Of these attributes, the most important is `rvw`, which stores the [ball state](2020-12-20-pooltool-alg.md/#what-is-the-system-state) as a $3 \times 3$ `numpy` array. `rvw` is named after the 3 state vectors $\vec{r}(t)$, $\vec{v}(t)$, and $\vec{\omega}(t)$.
...
This method ultimately calls upon [`physics.cue_strike`](https://github.com/ekiefl/pooltool/blob/51552ff7704376682359059b5dbd8a093f4ded17/psim/physics.py#L102), which implements the cue-ball interaction physics described [here](2020-04-24-pooltool-theory.md/#section-vi-ball-cue-interactions).
</div>
...
Rather than implementing the event-based shot evolution algorithm I [wouldn't shut up about](2020-12-20-pooltool-alg.md/#continuous-event-based-evolution), I implemented some dinky discrete time evolution algorithm just to get the _ball rolling_. It's a for loop that increments by $50 \text{ms}$.
...
So far I've been evolving the simulation by incrementing time in small discrete steps (_aka_ a [discrete time evolution algorithm](2020-12-20-pooltool-alg.md/#discrete-time-evolution). Yet moving forward, I've opted to use the event-based evolution algorithm for its superior accuracy and computational efficiency.

The premise of the algorithm is this:

- We have beautiful equations of motion for each ball that collectively describe the evolution of the [system state](2020-12-20-pooltool-alg.md/#what-is-the-system-state). Great.
...
- So the algorithm works by evolving the system state directly up until the next event, at which time the event must be resolved (_e.g._ a [ball-ball collision event](2020-12-20-pooltool-alg.md/#ball-ball-collision) is resolved by applying the [ball-ball interaction equations](2020-04-24-pooltool-theory.md/#section-ii-ball-ball-interactions)), and then the process repeats itself: the next event is found and the system state is evolved up until the next event.
...
How do these results compare to the discrete time algorithm? With discrete time, collisions are [detected retrospectively](2020-12-20-pooltool-alg.md/#discrete-time-evolution) by seeing if there is any overlapping geometry. This leads to an inherent inaccuracy. Sure, it can be reduced by **decreasing the time step**. But how small does the timestep have to be and **what effect does this have on performance**?
